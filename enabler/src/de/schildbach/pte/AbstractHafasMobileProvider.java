...
import de.schildbach.pte.dto.JourneyDetails;
...
public abstract class AbstractHafasMobileProvider extends AbstractHafasProvider {
	...
	public List<String> messageWhitelist = Arrays.asList("RT");
	...
	public QueryJourneyDetailResult queryJourneyDetails(String id, Date time) throws IOException {
		return jsonJourneyDetails(id, time);
	}
	...
	protected final NearbyLocationsResult jsonLocGeoPos(final EnumSet<LocationType> types, final int lat, final int lon)
		...
		try {
			...
			if (headErr != null && !headErr.equals("OK"))
			...
			parseRemList(common.getJSONArray("remL"));
	...
	protected final QueryDeparturesResult jsonStationBoard(final String stationId, final @Nullable Date time,
		...
		final CharSequence getPasslist = Boolean.toString(false); // traffic expensive
		...
		try {
			...
			if (jnyList != null) {
				for (int iJny = 0; iJny < jnyList.length(); iJny++) {
					...
					final String jid = jny.getString("jid");
					...
					final boolean depatureCanceled = jny.optBoolean("isCncl");
					...
					final JSONArray remList = jny.optJSONArray("msgL");
					...
					if (remList != null) {
						for (int iRem = 0; iRem < remList.length(); iRem++) {
							...
							if (remX > -1) {
								final String[] remark = remarks.get(rem.optInt("remX"));

								if (messageWhitelist.contains(remark[0]))
									messageList.add(remark[1]);
							}
						}
					}

					String message = Strings.emptyToNull(JOINER_NEW_LINE.join(messageList));

					final Departure departure = new Departure(plannedTime, predictedTime, line, position, destination, null, message, jid);
	...
	protected QueryJourneyDetailResult jsonJourneyDetails(String id, Date time) throws IOException {

		final String request = wrapJsonApiRequest("JourneyDetails",
				"{" + "\"jid\": \"" + id + "\"," + "\"getPolyline\":false,\"getPasslist\":true }",
				false);
		...
		try {
			...
			final JSONObject journey = res.getJSONObject("journey");

			final Line line = lines.get(journey.getInt("prodX"));

			final String dirTxt = journey.optString("dirTxt", null);
			final Location destination = dirTxt != null ? new Location(LocationType.STATION, null, null, dirTxt) : null;

			c.clear();
			ParserUtils.parseIsoDate(c, journey.getString("date"));
			final Date baseDate = c.getTime();

			final JSONArray stopList = journey.getJSONArray("stopL");
			checkState(stopList.length() >= 2);

			final JSONObject stopFirst = stopList.getJSONObject(0);
			final Stop firstStop = parseJsonStop(stopFirst, locations, c, baseDate);

			final JSONObject stopLast = stopList.getJSONObject(stopList.length() - 1);
			final Stop lastStop = parseJsonStop(stopLast, locations, c, baseDate);

			final List<Stop> allStops = new ArrayList<>(stopList.length());
			for (int iStop = 1; iStop < stopList.length() - 1; iStop++) {
				final JSONObject stop = stopList.getJSONObject(iStop);
				final Stop oneStop = parseJsonStop(stop, locations, c, baseDate);
				allStops.add(oneStop);
			}

			final JSONArray remList = journey.optJSONArray("msgL");
			ArrayList<String> messageList = new ArrayList<>();
			if (remList != null) {
				for (int iRem = 0; iRem < remList.length(); iRem++) {
					final JSONObject rem = remList.getJSONObject(iRem);
...
//						if (messageWhitelist.contains(remark[0]))
			...
			final Leg.Public leg = new Leg.Public(line, destination, firstStop, lastStop, allStops, null, message);
			final JourneyDetails journeyDetails = new JourneyDetails(leg, id);

			return new QueryJourneyDetailResult(header, journeyDetails);
	...
	protected final QueryTripsResult jsonTripSearch(Location from, @Nullable Location via, Location to, final Date time,
		...
		final CharSequence outFrwdKey = "1.18".equals(apiVersion) ? "outFrwd" : "frwd";
		...
		try {
			...
			for (int iOutCon = 0; iOutCon < outConList.length(); iOutCon++) {
				...
				final List<Leg> legs = new ArrayList<>(secList.length());
				for (int iSec = 0; iSec < secList.length(); iSec++) {
					...
					if ("JNY".equals(secType)) {
						...
						leg = new Leg.Public(line, destination, departureStop, arrivalStop, intermediateStops, null,
								message);
						...
						leg = new Leg.Individual(Leg.Individual.Type.WALK, departureStop.location,
	...
	private String wrapJsonApiRequest(final String meth, final String req, final boolean formatted) {
		return "{" //
				...
				+ "\"ver\":\"" + checkNotNull(apiVersion) + "\",\"lang\":\"deu\"," //
